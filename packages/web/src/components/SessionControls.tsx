...
import { type CookPresetChannel, CookPresetPicker } from "./CookPresetPicker.tsx";
...
export function SessionControls({
	...
	const {
		...
		recordSessionLabel,
		error,
	} = useSession(client, serial, sessionStart, sessionLabel);
	...
	const handlePresetApplied = (presetLabel: string, startedAt: Date | null) => {
		if (startedAt) {
			recordStartedSession(presetLabel, startedAt);
		} else {
			recordSessionLabel(presetLabel);
		}
	};

	return (
		<div className={cn("mt-3", isActive && "rounded-md border border-border bg-muted/50 p-2.5")}>
			{isActive ? (
				<>
			...
			<CookPresetPicker
				...
				onSessionApplied={handlePresetApplied}
			/>
